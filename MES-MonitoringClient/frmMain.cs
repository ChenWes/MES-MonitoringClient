...
namespace MES_MonitoringClient
{
    public partial class frmMain : Form
    {
        ...
        //状态操作类
        public static Common.MachineStatusHandler mc_MachineStatusHander = null;
            ...
            try
            {
                ...
                else
                {
                    if (MessageBox.Show("退出系统后，暂时不会收集到机器的数据，请知悉", "系统退出提醒", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
                    {
                        ...
                        //结束计数
                        if (MachineProductionThreadClass != null && MachineProductionTimerClass != null)
                        {
        ...
        /// 定时结束计数
        /// </summary>
        private void MachineProductionTimer()
        {
            //10s
            autoMachineProduction();
            MachineProductionTimerClass = new Common.TimmerHandler(10 * 1000, true, (o, a) =>
            {
                autoMachineProduction();
            }, true);
        ...
        private void autoMachineProduction()
        {
            ...
            List<DataModel.MachineProduction> machineProduction = machineProductionHandler.findAllRecord();
            DateTime now = DateTime.Now;
            if (machineProduction.Count > 0)
            {
                foreach (var item in machineProduction)
                {
                    BsonDocument bsons = item.ToBsonDocument();
                    DateTime endDateTime= mc_MachineStatusHander.mc_MachineProduceStatusHandler.findDateTime(item.WorkShiftID,item.Date,now,2);
                    if (endDateTime < now)
                    {
                        //结束员工记录
                        int i = 0;
                        foreach (var employeeProductionTimeList in item.EmployeeProductionTimeList.ToArray())
                        {
                            ...
                            if (employeeProductionTimeList.StartTime == employeeProductionTimeList.EndTime)
                            {
                                ...
                                //计算员工工时
                                item.EmployeeProductionTimeList[i].WorkHour = Math.Round((endDateTime- item.EmployeeProductionTimeList[i].StartTime.ToLocalTime()).TotalHours,3);
                            }
                            i++;
                        }
                        //结束工单记录
                    ...
                    /* else
                     {
                         ...
                         //辅助进程，计算时间
                         machineProductionHandler.ComputingTime(item, bsons);
                }*/
                }
            }
           /* else
        ...
        private void btn_QC_Click(object sender, EventArgs e)
        {
            if (mc_MachineStatusHander.MachineStatusCode == Common.MachineStatus.eumMachineStatus.Produce.ToString())
            {
                ...
                if (!newfrmScanRFID.MC_IsManualCancel && newfrmScanRFID.QCCheckCounts.Count > 0)
                {
                    ...
                    //更新工单和生产数不良品
                    mc_MachineStatusHander.mc_MachineProduceStatusHandler.SettingProductErrorCount(newfrmScanRFID.errorCount, newfrmScanRFID.QCTime);
                }
            }
            else
            {
