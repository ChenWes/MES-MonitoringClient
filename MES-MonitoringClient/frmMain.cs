...
namespace MES_MonitoringClient
{
	public partial class frmMain : Form
	{
		...
		private void SetDateTime()
		{
			try
			{
				...
				else
				{
					try
					{
						lab_DateTime.Text = string.Format("CurrentTime：" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
						...
						if (getServiceStatus == Common.CommonFunction.ServiceStatus.NoInstall)
						{
							...
							lab_UploadDataServiceStatus.Text = "The background upload service is running normally";
		...
		private void UpdateMachineNondefectiveCount()
		{
			...
			else
			{
				if (mc_MachineStatusHander.mc_MachineProduceStatusHandler.ProcessJobOrderList != null && mc_MachineStatusHander.mc_MachineProduceStatusHandler.CurrentProcessJobOrder != null)
				{
					...
					//同一模具切换工单
					//if (txt_ProductTotalCount.Text == txt_WorkOrderCount.Text && txt_NondefectiveCount.Text == txt_WorkOrderCount.Text)
					//{
					Automatic_CompletedJobOrd();
					//}
			...
			this.Invoke(new Action(() =>
			{
				...
				if (newVersion != "" && !(newVersion is null))
				{
					...
					else
					{
						...
						this.lab_log.Text = "Update application";
		...
		//工单同一模具自动切换工单(完成工单)
		private void Automatic_CompletedJobOrd()
		{
			try
			{
				frmScanRFID newfrmScanRFID = new frmScanRFID();
				...
				string CurrentMouldCold = txt_MouldCode.Text;
				string CurrentxtMaterialCode = txt_MaterialCode.Text;

				//完成工单
				mc_MachineStatusHander.mc_MachineProduceStatusHandler.AutomaticCompleteJobOrder();

				

				//List<DataModel.JobOrderDisplay> jobOrders = Common.JobOrderHelper.GetJobOrderByMouldCode(CurrentMouldCold, "1");
				List<DataModel.JobOrder> jobOrders_1 = Common.JobOrderHelper.GetJobOrderByMouldCode_1(CurrentMouldCold, "1");

				//获取订单
				List<DataModel.JobOrder> jobOrderList = jobOrders_1;
				if (jobOrderList == null || jobOrderList.Count == 0) throw new Exception("未选择工单！");
				
				mc_MachineStatusHander.mc_MachineProduceStatusHandler.SetJobOrder1(jobOrders_1);
			

				//找到生产中状态
				DataModel.MachineStatus machineStatus = Common.MachineStatusHelper.GetMachineStatusByCode(Common.MachineStatus.eumMachineStatus.Produce.ToString());

				mc_MachineStatusHander.ChangeStatus1(
					machineStatus._id,
					machineStatus.MachineStatusCode,
					machineStatus.MachineStatusName,
					machineStatus.MachineStatusDesc,
			...
			{
				Common.LogHandler.WriteLog("切换模具工单错误", ex);
			}
