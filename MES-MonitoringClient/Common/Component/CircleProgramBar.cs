...
namespace MES_MonitoringClient.Common.Component
{
    public class CircleProgramBar : Control
    {
        ...
        public CircleProgramBar()
        {
            InitControl();
            //初始化变量
            this.penBottom = new Pen(this.bottomColor, 10);
            this.penTop = new Pen(this.topColor, 10);
            this.penFinished = new Pen(this.finishedColor, 10);
        ...
        private void DrawShape(Graphics g)
        {
           ...
           // this.penBottom.Width = (size - (sizeOffset * 2));
            ...
            if (this.progress > maxValue)
            {
                ...
                g.DrawArc(new Pen(Color.Red, 10), rectangle, 0, (int)topAngle);//绘制进度条
                SizeF proValSize = g.MeasureString("超"+(progress-maxValue), this.Font);//计算文字范围
                g.DrawString("超" + (progress - maxValue), this.Font, new SolidBrush(Color.Red), rectangle.X + rectangle.Width / 2 - proValSize.Width / 2, rectangle.Y + rectangle.Height / 2 - proValSize.Height / 2);
            }
            else 
            {
                //绘制进度值
